...
def menu():
    ...
    print("The program is made by TCP agreement.This is the TCP Server.")
    print("menu:")
...
def handle_server(client_server):
    while True:
        ...
        if option == "1":
            send_server(client_server)
        elif option == "2":
            recv_server(client_server)
        elif option == "3":
...
def main():
    ...
    server_socket.bind(("", 9546))
    client_message = {}
    # get host name
    host_name = socket.gethostname()
    # get computer ip address
    ip_address = socket.gethostname(host_name)
    print("{} ip address is {}".format(host_name, ip_address))
    server_socket.listen(10)
    print("Waiting for connect...")
    while True:
        client_socket, client_ip_port = server_socket.accept()
        print("{} connected.".format(client_ip_port))
        client_message[client_ip_port] = client_socket  # 保存所有跟服务端建立连接的客户端的信息
        process = multiprocessing.Process(target=handle_server, args=(handle_server,), daemon=True)
        process.start()
        # 增加一个条件，判断满足时跳出循环，结束程序
    server_socket.close()
    print("Connect end.")
